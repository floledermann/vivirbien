    ...
    def forwards(self, orm):
        for resource in orm.Resource.objects.all():
            if resource.start_date:
                tag = orm.Tag(key='start_date', value=resource.start_date.__str__(), value_date=resource.start_date, resource=resource, creator=resource.creator)
                tag.save()
            if resource.end_date:
                tag = orm.Tag(key='end_date', value=resource.end_date.__str__(), value_date=resource.end_date, resource=resource, creator=resource.creator)
                tag.save()           
